...
from provider_manager import GestorProveedores
from providers.base import MensajeResumen
...
class ItemDireccion(QListWidgetItem):
    ...
    def actualizar_texto(self):
        ...
        restante = utilities.minutos_restantes_caducidad(
...
class VentanaPrincipal(QMainWindow):
    def __init__(self):
        super().__init__()
        self.configuracion = configuration.cargar_configuracion()
        languages.establecer_idioma(self.configuracion.get("idioma", "es"))
        ...
        self.cuentas = storage.cargar_cuentas()
        self.historial = storage.cargar_historial()
    ...
    def _avisar_si_hubo_problemas_de_almacenamiento(self):
        if not storage.advertencias_carga:
            return
        detalle = "\n\n".join(storage.advertencias_carga)
        QMessageBox.warning(self, t("titulo_advertencia_almacenamiento"), detalle)
        storage.advertencias_carga.clear()
    ...
    def _aplicar_tema(self, nombre_tema):
        nombre_archivo = "dark_theme.qss" if nombre_tema == "oscuro" else "light_theme.qss"
    ...
    def _al_crear_cuenta_exito(self, datos_cuenta, identificador_proveedor):
        ...
        datos_cuenta["creado"] = utilities.marca_de_tiempo_actual()
        datos_cuenta["creado_iso"] = utilities.marca_de_tiempo_iso_actual()
        self.cuentas.append(datos_cuenta)
        storage.guardar_cuentas(self.cuentas)
    ...
    def _accion_eliminar_cuenta(self):
        ...
        storage.guardar_cuentas(self.cuentas)
        storage.eliminar_historial_de_direccion(self.historial, cuenta["address"])
        storage.guardar_historial(self.historial)
    ...
    def _refrescar_tabla_mensajes(self):
        ...
        for mensaje in self.mensajes_actuales:
            ...
            fecha = utilities.formatear_fecha_mensaje(mensaje.fecha_iso)
    ...
    def _registrar_en_historial_si_procede(self):
        ...
        if cuenta is None:
            return
        storage.registrar_mensajes_en_historial(
    ...
    def _detectar_y_mostrar_codigo(self, asunto, cuerpo):
        ...
        codigo = utilities.extraer_codigo_verificacion(
    ...
    def _alternar_vista_historial(self, activo):
        ...
        if activo:
            ...
            entradas = storage.historial_de_direccion(self.historial, cuenta["address"])
    ...
    def _al_aplicar_ajustes(self, nueva_configuracion):
        ...
        configuration.guardar_configuracion(self.configuracion)
        ...
        if idioma_cambio:
            languages.establecer_idioma(self.configuracion["idioma"])
...
def main():
    config_inicial = configuration.cargar_configuracion()
    languages.establecer_idioma(config_inicial.get("idioma", "es"))
